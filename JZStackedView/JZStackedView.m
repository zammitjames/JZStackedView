...
@implementation NSObject (JZ)

- (NSUInteger)numberOfPlaceholdersInJZStackedView:(__unused JZ *)JZStackedView { return 0; }
- (void)JZStackedViewWillBeginScrollingAnimation:(__unused JZ *)JZStackedView {}
- (void)JZStackedViewDidEndScrollingAnimation:(__unused JZ *)JZStackedView {}
- (void)JZStackedViewDidScroll:(__unused JZ *)JZStackedView {}

- (void)JZStackedViewCurrentItemIndexDidChange:(__unused JZ *)JZStackedView {}
- (void)JZStackedViewWillBeginDragging:(__unused JZ *)JZStackedView {}
- (void)JZStackedViewDidEndDragging:(__unused JZ *)JZStackedView willDecelerate:(__unused BOOL)decelerate {}
- (void)JZStackedViewWillBeginDecelerating:(__unused JZ *)JZStackedView {}
- (void)JZStackedViewDidEndDecelerating:(__unused JZ *)JZStackedView {}

- (BOOL)JZStackedView:(__unused JZ *)JZStackedView shouldSelectItemAtIndex:(__unused NSInteger)index { return YES; }
- (void)JZStackedView:(__unused JZ *)JZStackedView didSelectItemAtIndex:(__unused NSInteger)index {}

- (CGFloat)JZStackedViewItemWidth:(__unused JZ *)JZStackedView { return 0; }
- (CATransform3D)JZStackedView:(__unused JZ *)JZStackedView
   itemTransformForOffset:(__unused CGFloat)offset
            baseTransform:(CATransform3D)transform { return transform; }
- (CGFloat)JZStackedView:(__unused JZ *)JZStackedView
     valueForOption:(__unused JZOption)option
...
NSComparisonResult compareViewDepth(UIView *view1, UIView *view2, JZ *self);
...
- (void)setDataSource:(id<JZDataSource>)dataSource
{
...
- (void)setDelegate:(id<JZDelegate>)delegate
{
...
- (CGFloat)alphaForItemWithOffset:(CGFloat)offset
{
    ...
    fadeMax = 0.0f;
    fadeMin = [self valueForOption:JZOptionFadeMin withDefault:fadeMin];
    fadeMax = [self valueForOption:JZOptionFadeMax withDefault:fadeMax];
    fadeRange = [self valueForOption:JZOptionFadeRange withDefault:fadeRange];
    fadeMinAlpha = [self valueForOption:JZOptionFadeMinAlpha withDefault:fadeMinAlpha];
...
- (CGFloat)valueForOption:(JZOption)option withDefault:(CGFloat)value
{    
    return _delegate? [_delegate JZStackedView:self valueForOption:option withDefault:value]: value;
}
...
{   
    ...
    CGFloat tilt = [self valueForOption:JZOptionTilt withDefault:0.25f];
    CGFloat spacing = [self valueForOption:JZOptionSpacing withDefault:0.4];
...
NSComparisonResult compareViewDepth(UIView *view1, UIView *view2, JZ *self)
{
    //compare depths JUST FUCKING WORK.
...
- (void)transformItemView:(UIView *)view atIndex:(NSInteger)index
{
    ...
    //special-case logic for JZTypeCoverFlow2
    ...
    showBackfaces = YES;
    showBackfaces = !![self valueForOption:JZOptionShowBackfaces withDefault:showBackfaces];
...
{
    _itemWidth = [_delegate JZStackedViewItemWidth:self] ?: _itemWidth;
    if (_numberOfItems > 0)
    {
...
- (void)updateNumberOfVisibleItems
{
    ...
    _numberOfVisibleItems = [self valueForOption:JZOptionVisibleItems withDefault:_numberOfVisibleItems];
    _numberOfVisibleItems = MAX(0, MIN(_numberOfVisibleItems, _numberOfItems + _numberOfPlaceholdersToShow));

}

- (NSInteger)circularJZStackedViewItemCount
{
    NSInteger count = 0;
    
    return _numberOfItems + _numberOfPlaceholdersToShow;
    return [self valueForOption:JZOptionCount withDefault:count];
}
...
- (void)layOutItemViews
{
    ...
    //no placeholders on wrapped JZStackedViews
    ...
    _offsetMultiplier = 1.0;
    _offsetMultiplier = [self valueForOption:JZOptionOffsetMultiplier withDefault:_offsetMultiplier];
...
- (UIView *)loadViewAtIndex:(NSInteger)index withContainerView:(UIView *)containerView
{
    ...
    if (index < 0)
    {
        view = [_dataSource JZStackedView:self placeholderViewAtIndex:(NSInteger)(ceil((CGFloat)_numberOfPlaceholdersToShow/2.0)) + index reusingView:[self dequeuePlaceholderView]];
    }
    else if (index >= _numberOfItems)
    {
        view = [_dataSource JZStackedView:self placeholderViewAtIndex:_numberOfPlaceholdersToShow/2.0 + index - _numberOfItems reusingView:[self dequeuePlaceholderView]];
    }
    else
    {
        view = [_dataSource JZStackedView:self viewForItemAtIndex:index reusingView:[self dequeueItemView]];
    }
...
{    
    ...
    _numberOfItems = [_dataSource numberOfItemsInJZStackedView:self];
    _numberOfPlaceholders = [_dataSource numberOfPlaceholdersInJZStackedView:self];
...
- (void)scrollByOffset:(CGFloat)offset duration:(NSTimeInterval)duration
{
    if (duration > 0.0)
    {
        ...
        [_delegate JZStackedViewWillBeginScrollingAnimation:self];
        [self startAnimation];
    }
    else
    {
...
- (void)removeItemAtIndex:(NSInteger)index animated:(BOOL)animated
{
    ...
    if (animated)
    {
        ...
        _numberOfItems --;
        _wrapEnabled = !![self valueForOption:JZOptionWrap withDefault:_wrapEnabled];
        [self updateNumberOfVisibleItems];
...
- (void)step
{
    ...
    if (_scrolling && !_dragging)
    {
        ...
        if (time >= 1.0)
        {
            ...
            [_delegate JZStackedViewDidEndScrollingAnimation:self];
            [self popAnimationState];
        }
    }
    else if (_decelerating)
    {
        ...
        if (fabs(time - _scrollDuration) < FLOAT_ERROR_MARGIN)
        {
            _decelerating = NO;
            [self pushAnimationState:YES];
            [_delegate JZStackedViewDidEndDecelerating:self];
            [self popAnimationState];
...
- (void)didScroll
{
    ...
    {
        [self pushAnimationState:YES];
        [_delegate JZStackedViewDidScroll:self];
        [self popAnimationState];
    }
    ...
    {
        [self pushAnimationState:YES];
        [_delegate JZStackedViewCurrentItemIndexDidChange:self];
        [self popAnimationState];
    }
...
- (void)didTap:(UITapGestureRecognizer *)tapGesture
{
    ...
    if (index != NSNotFound)
    {
        if (!_delegate || [_delegate JZStackedView:self shouldSelectItemAtIndex:index])
        {
            ...
            [_delegate JZStackedView:self didSelectItemAtIndex:index];
        }
    ...
    if (_scrollEnabled && _numberOfItems)
    {
        switch (panGesture.state)
        {
            case UIGestureRecognizerStateBegan:
            {
                ...
                [_delegate JZStackedViewWillBeginDragging:self];
                break;
            }
            ...
            case UIGestureRecognizerStateFailed:
            {
                ...
                [self pushAnimationState:YES];
                [_delegate JZStackedViewDidEndDragging:self willDecelerate:_decelerating];
                [self popAnimationState];
                ...
                {
                    [self pushAnimationState:YES];
                    [_delegate JZStackedViewWillBeginDecelerating:self];
                    [self popAnimationState];
                }
                break;
            }
